...
function cleanPdfText(rawText: string): string {
  ...
  text = text.replace(
    /([a-zA-Z])(\s{1,2})([a-zA-Z])(\s{1,2})([a-zA-Z])/g,
    '$1$3$5'
  );
  ...
  text = text.replace(
    /([a-z])([A-Z][a-z]+)(Skills|Experience|Education|Projects|Professional|Technical|Cloud|Frontend|Backend|Databases)/g,
    '$1\n$2$3'
  );
  ...
  text = text.replace(
    /([a-zA-Z0-9])\s{0,1}(\|)\s{0,1}([A-Z][a-z]{2}\d{4}|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)/g,
    '$1 $2 $3'
  );
  text = text.replace(
    /([A-Z][a-z]{2}\d{4}\s*(?:–|—|-)\s*(?:Present|[A-Z][a-z]{2}\d{4}))/g,
    '\n$1\n'
  );
    ...
    try {
      ...
      pdfParser.on('pdfParser_dataError', (data) => {
        const errorMsg =
          typeof data === 'string'
            ? data
            : (data as { parserError?: string })?.parserError ||
      ...
      pdfParser.on('pdfParser_dataReady', (pdfData) => {
        ...
        if (pdfData.Pages) {
          pdfData.Pages.forEach(
            (page: { Texts?: Array<{ R?: Array<{ T: string }> }> }) => {
              if (page.Texts) {
                page.Texts.forEach((textObj: { R?: Array<{ T: string }> }) => {
                  if (textObj.R && textObj.R.length > 0) {
                    textObj.R.forEach((r: { T: string }) => {
                      try {
...
export async function POST(request: NextRequest) {
  try {
    ...
    log.info(
      { fileName: file.name, textLength: text.length },
