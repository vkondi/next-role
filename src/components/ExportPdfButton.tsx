...
export default function ExportPdfButton({
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await generateCareerStrategyPDF({
        resumeProfile,
        selectedPath,
        skillGapAnalysis,
        roadmap,
  ...
  const handleShare = (
    ...
    shareOnSocialMedia(platform, {
      roleName: selectedPath.roleName,
  ...
  return (
    <div className="card text-center space-y-3 sm:space-y-4">
      ...
      <p className="text-body text-slate-600 text-sm sm:text-base">
        Download your personalized career strategy or share it with others.
      </p>
        ...
        <div className="relative w-full sm:w-auto">
          ...
          {showShareMenu && (
            <>
              ...
              />
              <div className="absolute right-0 sm:left-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 py-2 z-50 max-h-80 overflow-y-auto">
                <button
      ...
      <p className="text-xs text-slate-500 mt-2">
        PDF includes your profile, selected path, skill gaps, and complete
        roadmap
      </p>
    </div>
  );
