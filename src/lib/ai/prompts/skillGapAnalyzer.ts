...
import type { ResumeProfile, CareerPath, SkillGapAnalysis } from '../schemas';
 ...
 */
function tryRecoverSkillGapAnalysis(
  text: string
  ...
  if (overallGapSeverity) {
    const normalized = normalizeEnumValue(overallGapSeverity, [
      'Low',
      'Medium',
      'High',
    ]);
  ...
  if (estimatedTimeToClose)
    recovered.estimatedTimeToClose = estimatedTimeToClose;
  ...
  if (skillGapsMatch) {
    ...
    if (skillMatches.length > 0) {
      recovered.skillGaps = skillMatches.map((skillStr) => {
        ...
        if (currentMatch) {
          const normalized = normalizeEnumValue(currentMatch[1].split('"')[0], [
            'None',
            'Beginner',
            'Intermediate',
            'Advanced',
        ...
        const requiredMatch = skillStr.match(
          /"requiredLevel"\s*:\s*"([^"]*)"/i
        );
        if (requiredMatch) {
...
export async function parseSkillGapAnalyzerResponse(
  ...
  try {
    ...
    if (parsed.overallGapSeverity) {
      const normalized = normalizeEnumValue(parsed.overallGapSeverity, [
        'Low',
        'Medium',
        'High',
      ]);
    ...
    if (Array.isArray(parsed.skillGaps)) {
      parsed.skillGaps = parsed.skillGaps.map(
        (gap: Record<string, unknown>) => {
          const importance = normalizeEnumValue(gap.importance, [
            'Low',
            'Medium',
            'High',
          ]);
      ...
      (parsed.skillGaps as Array<Record<string, unknown>>).sort(
        (a: Record<string, unknown>, b: Record<string, unknown>) => {
          const aPriority =
            importancePriority[
              a.importance as keyof typeof importancePriority
            ] || 0;
          const bPriority =
            importancePriority[
              b.importance as keyof typeof importancePriority
            ] || 0;
