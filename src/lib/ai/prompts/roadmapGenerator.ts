...
function getRecommendedPhaseCount(
  ...
  const severity =
    overallGapSeverity as keyof typeof ROADMAP_CONFIG.PHASE_COUNT_BY_SEVERITY;
...
${Array.from({ length: recommendedPhases }, (_, i) => {
  ...
  const endMonth =
    i === recommendedPhases - 1 ? timelineMonths : (i + 1) * monthsPerPhase;
 ...
 */
function tryRecoverRoadmap(text: string): Record<string, unknown> | null {
  ...
  if (phasesMatch) {
    ...
    if (phaseMatches.length > 0) {
      recovered.phases = phaseMatches.map((phaseStr, idx) => {
        ...
        const projectsMatch = phaseStr.match(
          /"projectIdeas"\s*:\s*\[([^\]]*)\]/i
        );
        if (projectsMatch) {
        ...
        const milestonesMatch = phaseStr.match(
          /"milestones"\s*:\s*\[([^\]]*)\]/i
        );
        if (milestonesMatch) {
...
export async function parseRoadmapGeneratorResponse(
  ...
  try {
    const cleanedText = removeMarkdownBlocks(responseText.trim());
    ...
    try {
      ...
      log.debug(
        {
          phasesCount: Array.isArray(parsed.phases) ? parsed.phases.length : 0,
        },
      ...
      try {
        ...
        log.debug(
          {
            phasesCount: Array.isArray(parsed.phases)
              ? parsed.phases.length
              : 0,
          },
          'AI response repaired and parsed'
        );
      } catch {
        ...
        log.debug(
          ...
          'AI response recovered via regex'
        );
      }
    }
    ...
    if (
      ...
      log.warn(
        ...
        'No phases in AI response, using fallback'
      );
      ...
      const timelineMonths = (parsed.timelineMonths as number) || 6;
      const recommendedPhases = getRecommendedPhaseCount(
        (parsed.gapSeverity as 'Low' | 'Medium' | 'High') || 'Medium',
        timelineMonths
      );
      ...
      log.warn(
        { phaseCount: (parsed.phases as Array<unknown>).length },
        'Generated default phases as fallback'
      );
    }
    ...
    const result = validated as typeof validated & { _usedFallback?: boolean };
    ...
    log.info(
      {
        ...
        usedFallback: !!result._usedFallback,
      },
      'Roadmap parsed and validated'
    );
    return result;
  } catch (error) {
    log.error(
...
function generateDefaultPhases(
  ...
  for (let i = 0; i < phaseCount; i++) {
    const startMonth = i * monthsPerPhase + 1;
    const endMonth =
      i === phaseCount - 1 ? timelineMonths : (i + 1) * monthsPerPhase;

    const phaseDescriptions = [
      { direction: 'Build foundational skills', focus: 'Core fundamentals' },
      {
        direction: 'Develop intermediate expertise',
        focus: 'Practical application',
      },
      { direction: 'Advance specialized skills', focus: 'Advanced concepts' },
      {
        direction: 'Prepare for transition',
        focus: 'Portfolio and preparation',
      },
