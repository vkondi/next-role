...
export function createResumeInterpreterPrompt(resumeText: string): string {
...
export async function parseResumeInterpreterResponse(
  ...
  try {
    ...
    const normalized: Record<string, unknown> = {
      ...
      industryBackground:
        String(parsed.industryBackground ?? '').trim() || 'Industry',
    };

    // Ensure required arrays are not empty
    if (
      !Array.isArray(normalized.techStack) ||
      !(normalized.techStack as string[]).length
    )
      normalized.techStack = ['Not specified'];
    if (
      !Array.isArray(normalized.strengthAreas) ||
      !(normalized.strengthAreas as string[]).length
    )
      normalized.strengthAreas = ['Not specified'];
    ...
    throw new Error(`Failed to parse resume interpreter response: ${errorMsg}`);
  }
}
...
function tryRecoverJSON(text: string): string | null {
  ...
  const strengthMatch = /"strengthAreas"\s*:\s*\[(.*?)(?:\]|$)/.exec(jsonPart);
  if (strengthMatch) {
  ...
  const industryMatch = /"industryBackground"\s*:\s*"([^"]*)/.exec(jsonPart);
  if (industryMatch)
    extracted.industryBackground = industryMatch[1].split('"')[0];

  // Extract certifications (array - may be incomplete)
  const certMatch = /"certifications"\s*:\s*\[(.*?)(?:\]|$)/.exec(jsonPart);
  if (certMatch) {
  ...
  const eduMatch = /"education"\s*:\s*\[(.*?)(?:\]|$)/.exec(jsonPart);
  if (eduMatch) {
...
export async function interpretResume(
  resumeText: string,
